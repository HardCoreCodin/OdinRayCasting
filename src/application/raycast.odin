...
castRay :: proc(using ray: ^Ray) {
    ...
    horizontal_hit.position.y = f32(i32(position.y / size) * tile_map.tile_size) + (is_facing_down ? size : 0);
    ...
    vertical_hit.position.x = f32(i32(position.x / size) * tile_map.tile_size) + (is_facing_right ? size : 0);
